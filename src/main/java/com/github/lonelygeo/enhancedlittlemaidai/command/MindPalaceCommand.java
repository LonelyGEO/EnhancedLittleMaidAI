...
public final class MindPalaceCommand {
    ...
    private static int listMemories(CommandSourceStack src, String name) {
        EntityMaid maid = MaidNameLookup.find(name, src);
        if (maid == null) { src.sendFailure(Component.literal("未找到名为 \"" + name + "\" 的女仆")); return 0; }

        MindPalace palace = MindPalace.get(maid.getUUID());
        if (palace == null || palace.size() == 0) {
            src.sendSuccess(() -> Component.literal(maid.getDisplayName().getString() + " 没有记忆"), false);
            return 1;
        }
        ...
        String header = "--- " + maid.getDisplayName().getString() + " 的记忆 (共 " + items.size() + " 条) ---";
        src.sendSuccess(() -> Component.literal(header), false);

        int index = 1;
        for (MemoryItem m : items) {
            StringBuilder line = new StringBuilder();
            line.append("#").append(index++).append(" [").append(categoryName(m.category())).append("] ").append(m.content());

            StringBuilder meta = new StringBuilder();
            if (m.location().isPresent() || m.dimension().isPresent()) {
                meta.append("  (");
                m.location().ifPresent(loc -> meta.append(loc.getX()).append(",").append(loc.getY()).append(",").append(loc.getZ()));
                m.dimension().ifPresent(dim -> meta.append(" ").append(dim));
                meta.append(")");
            }
    ...
    private static int searchMemories(CommandSourceStack src, String name, String keyword) {
        EntityMaid maid = MaidNameLookup.find(name, src);
        if (maid == null) { src.sendFailure(Component.literal("未找到名为 \"" + name + "\" 的女仆")); return 0; }
        ...
        if (items.isEmpty()) {
            src.sendSuccess(() -> Component.literal(maid.getDisplayName().getString() + " 搜索 \"" + keyword + "\": 无结果"), false);
            return 1;
        }

        String header = maid.getDisplayName().getString() + " 搜索 \"" + keyword + "\" 结果 (前 " + items.size() + " 条):";
        src.sendSuccess(() -> Component.literal(header), false);

        for (MemoryItem m : items) {
            StringBuilder line = new StringBuilder();
            line.append("  [").append(categoryName(m.category())).append("] ").append(m.content());
            m.location().ifPresent(loc -> line.append(" (").append(loc.getX()).append(",").append(loc.getY()).append(",").append(loc.getZ()).append(")"));
            src.sendSuccess(() -> Component.literal(line.toString()), false);
        }
        return 1;
    }

    private static int clearMemories(CommandSourceStack src, String name) {
        EntityMaid maid = MaidNameLookup.find(name, src);
        if (maid == null) { src.sendFailure(Component.literal("未找到名为 \"" + name + "\" 的女仆")); return 0; }
        ...
        String msg = count > 0 ? "已清除 " + maid.getDisplayName().getString() + " 的 " + count + " 条记忆"
                : maid.getDisplayName().getString() + " 没有记忆";
        src.sendSuccess(() -> Component.literal(msg), false);
        return 1;
    }

    private static int showStats(CommandSourceStack src, String name) {
        EntityMaid maid = MaidNameLookup.find(name, src);
        if (maid == null) { src.sendFailure(Component.literal("未找到名为 \"" + name + "\" 的女仆")); return 0; }
        ...
        String result = maid.getDisplayName().getString() + " 记忆统计\n"
                + "共 " + store.size() + " 条"
                + " | 地点:" + place + " | 人物:" + person + " | 事件:" + event
                + " | 偏好:" + pref + " | 知识:" + knowledge;
